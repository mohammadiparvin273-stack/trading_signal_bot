...
from storage import history_store, outcome_tracker, run_log
...
class SignalPipeline:
    ...
    def process_symbol(self, asset: str, timeframe: Optional[str] = None,
        ...
        self._last_history = clean
        # 3. Feature Engineering
        feats = build_features(clean)
        self._last_price = float(feats.iloc[-1]["close"])
    ...
    def process_and_notify(self, asset: str, timeframe: Optional[str] = None) -> Optional[FinalSignal]:
        ...
        if should_send:
            ...
            # Start/adjust real outcome tracking (did it actually hit TP or SL?).
            if change_type == "NEW_SIGNAL":
                outcome_tracker.register_new_signal(signal)
            elif change_type in ("STOP_LOSS_UPDATE", "TAKE_PROFIT_UPDATE"):
                outcome_tracker.update_open_levels(signal)

        # Check any previously-open signals for this asset against the
        # latest persisted price history — did they hit TP1/SL/expire?
        history = getattr(self, "_last_history", None)
        if history is not None:
            outcome_tracker.evaluate_open_outcomes(signal.asset, signal.timeframe, history)
