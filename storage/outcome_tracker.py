...
def summary_stats() -> dict:
    ...
    # Win rate and average PnL are only meaningful over trades that actually
    # resolved (hit TP1, hit SL, or timed out) — a SUPERSEDED entry has no
    # pnl_pct (None) since it was replaced before resolving, so it must be
    # excluded from both the count and the average, not just skipped in the
    # sum (that mismatch is what caused the crash before).
